...
def ln_pre_row_macro(LEN, C: int, DTYPE: str):
    ...
    Shared by both prologues so the LN math lives in one place. ``x_row`` is
    a 1-D ``[C]`` view: the whole buffer for decode, or a read-only row slice
    ``x0[n, :]`` for multi. Writes the normalized row to ``x_ln[n, :]``.
    """
