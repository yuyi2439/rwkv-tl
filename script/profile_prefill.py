...
with profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA], record_shapes=False) as prof:
    with record_function("prefill_T32"):
        with torch.device("cuda"):
            model.forward_prefill(TOKENS, S)
...
for row in prof.key_averages():
    ...
    key = "other"
    nl = name.lower()
    if "gemv" in nl or "mv" in nl or "gemm" in nl or "mm" in nl or "addmv" in nl or "bmm" in nl:
        key = "GEMM/GEMV"
    elif "dplr" in nl or "fused_dplr" in nl:
        key = "DPLR"
    elif "lerp" in nl:
        key = "LERP"
    elif "gate" in nl or "sigmoid" in nl or "w_gate" in nl or "v_gate" in nl or "a_kk" in nl or "neg_kk" in nl:
        key = "gates"
    elif "norm" in nl or "gn_rkrk" in nl or "l2norm" in nl or "layer_norm" in nl or "group_norm" in nl or "fused_l2norm" in nl or "fused_gn" in nl:
        key = "norm"
    groups[key][0] += t
